...
void timer_RF_init(){ // TIM4
	...
	timer_init.Autoreload = 19999; //16bits 20000 * 72 = f pwm
	//pwm = 20ms = 144 mhz
	...
	//14.3.7 PWM input mode P315 BIT MAP P362 
	...
	//write the TS bits to 101 in the TIMx_SMCR register (TI1FP1 selected).
	TIM4->SMCR &= ~(7<<4);
	TIM4->SMCR |= 5<<4;
...
float get_vitesse_sens(){
	...
	//pulse=pulse;
	//duree de l'impulsion entre 1ms et 2ms donc pulse  entre 1000 et 2000
